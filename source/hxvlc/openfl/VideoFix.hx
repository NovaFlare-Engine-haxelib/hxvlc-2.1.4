...
static int media_open(void *opaque, void **datap, uint64_t *sizep)
{
	if (opaque)
	{
		...
		int result = reinterpret_cast<VideoFix_obj *>(opaque)->mediaOpen(sizep);
...
static ssize_t media_read(void *opaque, unsigned char *buf, size_t len)
{
	if (opaque)
	{
		...
		ssize_t bytesToRead = reinterpret_cast<VideoFix_obj *>(opaque)->mediaRead(buf, len);
...
static int media_seek(void *opaque, uint64_t offset)
{
	if (opaque)
	{
		...
		int success = reinterpret_cast<VideoFix_obj *>(opaque)->mediaSeek(offset);
...
static void *video_lock(void *opaque, void **planes)
{
	if (opaque)
	{
		...
		void *picture = reinterpret_cast<VideoFix_obj *>(opaque)->videoLock(planes);
...
static void video_unlock(void *opaque, void *picture, void *const *planes)
{
	if (opaque)
	{
		...
		reinterpret_cast<VideoFix_obj *>(opaque)->videoUnlock(planes);
...
static void video_display(void *opaque, void *picture)
{
	if (opaque)
	{
		...
		reinterpret_cast<VideoFix_obj *>(opaque)->videoDisplay(picture);
...
static unsigned video_format_setup(void **opaque, char *chroma, unsigned *width, unsigned *height, unsigned *pitches, unsigned *lines)
{
	if (opaque && (*opaque))
	{
		...
		int pictureBuffers = reinterpret_cast<VideoFix_obj *>(*opaque)->videoFormatSetup(chroma, width, height, pitches, lines);
...
static void audio_play(void *data, const void *samples, unsigned count, int64_t pts)
{
	if (data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(data)->audioPlay((unsigned char *)samples, count, pts);
...
static void audio_resume(void *data, int64_t pts)
{
	if (data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(data)->audioResume(pts);
...
static void audio_pause(void *data, int64_t pts)
{
	if (data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(data)->audioPause(pts);
...
static void audio_flush(void *data, int64_t pts)
{
	if (data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(data)->audioFlush(pts);
...
static int audio_setup(void **data, char *format, unsigned *rate, unsigned *channels)
{
	if (data && *data)
	{
		...
		int result = reinterpret_cast<VideoFix_obj *>(*data)->audioSetup(format, rate, channels);
...
static void audio_set_volume(void *data, float volume, bool mute)
{
	if (data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(data)->audioSetVolume(volume, mute);
...
static void event_manager_callbacks(const libvlc_event_t *p_event, void *p_data)
{
	if (p_data)
	{
		...
		reinterpret_cast<VideoFix_obj *>(p_data)->eventManagerCallbacks(p_event);

		hx::SetTopOfStack((int *)0, true);
	}
}')
class VideoFix extends openfl.display.Bitmap
{
	#if lime_openal
